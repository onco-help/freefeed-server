...
async function timelineToRSS(data, ctx) {
  ...
  const rss = builder.create('rss').att('version', '2.0');
  const channel = rss
    .ele('channel')
    .ele('title', {}, `${feedTitle} @ ${config.siteTitle}`)
    .up()
    .ele('link', {}, `${config.host}/${urlEscape(owner.username)}`)
    .up()
    .ele('description', {}, owner.description)
    .up();

  const userpic =
    owner.profilePictureLargeUrl || config.profilePictures.defaultProfilePictureMediumUrl;
  channel
    .ele('image')
    .ele('url', {}, userpic)
    .up()
  ...
  const postMakers = await Promise.all(
    data.timelines.posts.map((postId) => postItemMaker(postId, data, ctx)),
  );
...
async function postItemMaker(postId, data, ctx) {
  ...
  const userpic =
    author.profilePictureLargeUrl || config.profilePictures.defaultProfilePictureMediumUrl;
  const descriptionLines = [
    `<p class="freefeed-author">`,
    `<a href="${config.host}/${urlEscape(
      author.username,
    )}"><img src="${userpic}" width="50" height="50"></a>`,
    `<a href="${config.host}/${urlEscape(author.username)}"><strong>${htmlEscape(
      author.username,
    )}</strong></a>:`,
    `</p>`,
  ...
  if (
    ...
    for (const comment of comments) {
      if (
        +comment.createdAt - prevTime >= ommitBubblesThreshold ||
        comment.createdBy !== post.createdBy
      ) {
        break;
      }
      ...
      descriptionLines.push([
        `<div class="freefeed-comment" style="margin-left: 1em; margin-top: 2em;">${textToHTML(
          comment.body,
        )}</div>`,
      ]);
    }
  ...
  return (channel) => {
    const item = channel
      .ele('item')
      .ele('guid', {}, `freefeed:post:${post.id}`)
      .up()
      .ele('pubDate', {}, new Date(+post.createdAt).toGMTString())
      .up()
      .ele('link', {}, `${config.host}/${urlEscape(feedOwner.username)}/${urlEscape(post.id)}`)
      .up()
      .ele('author', {}, author.username)
      .up()
      .ele('title', {}, title)
      .up()
