...
describe('PrivateGroups', () => {
  ...
  describe('#admin', () => {
    ...
    beforeEach(async () => {
      [adminContext, nonAdminContext] = await funcTestHelper.createTestUsers(['Luna', 'yole']);
      group = await funcTestHelper.justCreateGroup(
        adminContext,
        'pepyatka-dev',
        'Pepyatka Developers',
        { private: true },
    ...
    it('should allow an administrator of private group to add another administrator', (done) => {
      request
        .post(
          `${app.context.config.host}/v1/groups/${group.username}/subscribers/${nonAdminContext.user.username}/admin`,
        )
  ...
  describe('#update', () => {
    ...
    beforeEach(async () => {
      context = await funcTestHelper.createTestUser('luna');
      group = await funcTestHelper.justCreateGroup(context, 'pepyatka-dev', 'pepyatka-dev', {
        isPrivate: true,
      });
    });
  ...
  describe('#unadmin', () => {
    ...
    beforeEach(async () => {
      ...
      await funcTestHelper.justCreateGroup(adminContext, 'pepyatka-dev', 'pepyatka-dev', {
        isPrivate: true,
      });
    });
  ...
  describe('#sendRequest', () => {
    ...
    beforeEach(async () => {
      [adminContext, nonAdminContext] = await funcTestHelper.createTestUsers(['Luna', 'yole']);
      group = await funcTestHelper.justCreateGroup(adminContext, 'pepyatka-dev', 'pepyatka-dev', {
        isPrivate: true,
      });
    });
    ...
    it('should not allow subscriber user to send subscription request to private group', (done) => {
      request
        ...
        .end(() => {
          request
            .post(
              `${app.context.config.host}/v1/groups/pepyatka-dev/acceptRequest${nonAdminContext.user.username}`,
            )
            .send({
              authToken: adminContext.authToken,
              _method: 'post',
  ...
  describe('subscription requests and membership management', () => {
    ...
    beforeEach(async () => {
      [adminContext, secondAdminContext, nonAdminContext, groupMemberContext] =
        await funcTestHelper.createTestUsers(['Luna', 'Neptune', 'yole', 'Pluto']);
      group = await funcTestHelper.justCreateGroup(adminContext, 'pepyatka-dev');
      ...
      await funcTestHelper.justCreatePost(adminContext, 'Post body', [group.username]);
    ...
    describe('#acceptRequest', () => {
      ...
      it('should reject nonexisting group', (done) => {
        request
          .post(
            `${app.context.config.host}/v1/groups/foobar/acceptRequest/${nonAdminContext.user.username}`,
          )
          .send({ authToken: adminContext.authToken })
          .end((err) => {
            err.should.not.be.empty;
      ...
      it('should not allow to accept non-existent subscription request', (done) => {
        request
          .post(
            `${app.context.config.host}/v1/groups/pepyatka-dev/acceptRequest/${groupMemberContext.user.username}`,
          )
          .send({ authToken: adminContext.authToken })
          .end((err) => {
            err.should.not.be.empty;
    ...
    describe('#rejectRequest', () => {
      it('should reject unauthenticated users', (done) => {
        request
          .post(
            `${app.context.config.host}/v1/groups/pepyatka-dev/rejectRequest/${nonAdminContext.user.username}`,
          )
          .end((err) => {
            err.should.not.be.empty;
            err.status.should.eql(401);
      ...
      it('should reject nonexisting group', (done) => {
        request
          .post(
            `${app.context.config.host}/v1/groups/foobar/rejectRequest/${nonAdminContext.user.username}`,
          )
          .send({ authToken: adminContext.authToken })
          .end((err) => {
            err.should.not.be.empty;
      ...
      it('should not allow to reject non-existent subscription request', (done) => {
        request
          .post(
            `${app.context.config.host}/v1/groups/pepyatka-dev/rejectRequest/${groupMemberContext.user.username}`,
          )
          .send({ authToken: adminContext.authToken })
          .end((err) => {
            err.should.not.be.empty;
    ...
    describe('#unsubscribeFromGroup', () => {
      it('should reject unauthenticated users', (done) => {
        request
          .post(
            `${app.context.config.host}/v1/groups/pepyatka-dev/unsubscribeFromGroup/${groupMemberContext.user.username}`,
          )
          .end((err) => {
            err.should.not.be.empty;
            err.status.should.eql(401);
      ...
      it('should reject nonexisting group', (done) => {
        request
          .post(
            `${app.context.config.host}/v1/groups/foobar/unsubscribeFromGroup/${groupMemberContext.user.username}`,
          )
          .send({ authToken: adminContext.authToken })
          .end((err) => {
            err.should.not.be.empty;
      ...
      it('should not allow to unsubscribe non-members from group', (done) => {
        request
          .post(
            `${app.context.config.host}/v1/groups/pepyatka-dev/unsubscribeFromGroup/${nonAdminContext.user.username}`,
          )
          .send({ authToken: adminContext.authToken })
          .end((err) => {
            err.should.not.be.empty;
      ...
      it('should not allow to unsubscribe admins from group', (done) => {
        request
          .post(
            `${app.context.config.host}/v1/groups/pepyatka-dev/unsubscribeFromGroup/${secondAdminContext.user.username}`,
          )
          .send({ authToken: adminContext.authToken })
          .end((err) => {
            err.should.not.be.empty;
      ...
      it('should not allow admins to unsubscribe theirself from group', (done) => {
        request
          .post(
            `${app.context.config.host}/v1/groups/pepyatka-dev/unsubscribeFromGroup/${adminContext.user.username}`,
          )
          .send({ authToken: adminContext.authToken })
          .end((err) => {
            err.should.not.be.empty;
    ...
    describe('v2/managedGroups', () => {
      it('should reject unauthenticated users', (done) => {
        request.get(`${app.context.config.host}/v2/managedGroups`).end((err) => {
          err.should.not.be.empty;
          err.status.should.eql(401);
          done();
        });
      ...
      it('requests array should match managed groups', (done) => {
        ...
        request
          ...
          .send({
            group: {
              username: 'pepyatka-dev-2',
              screenName: 'Pepyatka Developers 2',
              isPrivate: '1',
            },
            authToken: adminContext.authToken,
          })
            ...
            request
              ...
              .end((err, res) => {
                ...
                request
                  ...
                  .send({
                    group: {
                      username: 'pepyatka-dev-3',
                      screenName: 'Pepyatka Developers 3',
                      isPrivate: '1',
                    },
    ...
    describe('#whoami:pendingGroupRequests', () => {
      it('pendingGroupRequests should match managed groups', async () => {
        ...
        await Promise.all(
          contexts.map(({ context, shouldHave }) =>
            verifyUsersPendingGroupRequests(context, shouldHave),
          ),
        );
      });
    });
  ...
  describe('subscribers', () => {
    ...
    beforeEach(async () => {
      [adminContext, marsContext, plutoContext] = await funcTestHelper.createTestUsers([
        'Luna',
        'Mars',
        'Pluto',
      ]);
  ...
  describe('Posting to restricted group', () => {
    ...
    beforeEach(async () => {
      [adminContext, nonAdminContext, nonMemberContext] = await funcTestHelper.createTestUsers([
        'Luna',
        'yole',
        'Pluto',
      ]);
      ...
      const [group1, group2, group3] = await Promise.all([
        funcTestHelper.justCreateGroup(adminContext, 'pepyatka-dev', 'Pepyatka Developers', {
          isPrivate: true,
          isRestricted: true,
        }),
        funcTestHelper.justCreateGroup(adminContext, 'pepyatka-dev-2', 'Pepyatka Developers 2', {
          isPrivate: true,
          isRestricted: false,
        }),
        funcTestHelper.justCreateGroup(adminContext, 'pepyatka-dev-3', 'Pepyatka Developers 3', {
          isPrivate: false,
          isRestricted: true,
        }),
      ]);
            ...
            .end((err) => {
              ...
              request
                ...
                .send({
                  post: { body: 'Post body' },
                  meta: { feeds: 'pepyatka-dev' },
                  authToken: nonMemberContext.authToken,
                })
                .end((err) => {
                  err.should.not.be.empty;
                  err.status.should.eql(403);
